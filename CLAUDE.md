...
메타 서술(사람 이름·협업·부채·향후 작업·미구현 상태)을 파일·커밋에 작성하지 않고
보고 채널로만 전달하는 규칙은, 공통 규칙 `@AGENTS.md`의「파일 작성 금지 — 메타
서술 분리 원칙」 절을 **단일 진실 원천**으로 따른다. CLAUDE.md는 이 규칙을 중복
기술하지 않으며, 해당 절을 그대로 적용한다.
...
- **분기 위치**: API 클라이언트(`src/lib/api.ts`)에서 환경 변수 기준으로
  Mock/실제 API 분기
- **Mock 구조**: OpenAPI Spec v2.0.0의 스키마와 동일한 구조 유지
...
- `src/lib/schema.ts` — DB 스키마 단일 정의 (변경 시 `drizzle-kit` 마이그레이션
  필요)
