...
문서, 코드 주석, 환경변수 예시, README, 커밋 메시지 등을 작성할 때 사람의 이름을
직접 언급하지 않는다.

- 나쁜 예: "종민에게 문의", "현빈이 만든 X", "지영이 알고 있음"
- 좋은 예: "백엔드 담당자에게 문의", "프론트엔드 팀이 소유한 X", "팀 위키 참조"

이유: 사람은 이직하거나 역할이 바뀔 수 있지만, 역할과 참조 위치는 비교적
안정적이다. 문서는 작성자보다 오래 살아남으므로, 사람을 가리키면 시간이 지나
고장난 안내문이 된다.

이 규칙은 Gemini Code Assist의 PR 리뷰에서 발견된 패턴을 일반 규칙으로 추상화한
것이다.
