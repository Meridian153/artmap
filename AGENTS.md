...
- **아키텍처**: 풀스택 Next.js — 프론트엔드(App Router)와 백엔드(API Routes)를 하나의 Next.js 프로젝트로 처리. 별도 백엔드 서버 없음
- **배포**: Vercel 단일 배포
...
> **지도 import 경로 주의**: 반드시 `react-map-gl/maplibre` 사용.
> `react-map-gl` 직접 import 시 Mapbox(유료)로 연결됨.
...
- **TypeScript**: strict 모드. `any` 타입 사용 금지 (`unknown` 또는 구체적 타입 사용)
